...
Ещё не построены: детект по сохранённому потоку, реагирование и интерфейс. Порядок описан в [docs/roadmap.md](docs/roadmap.md).
...
То же самое, но каждая роль в своём контейнере, а между ними Redpanda, как
роли на разных хостах общались бы через Kafka:
